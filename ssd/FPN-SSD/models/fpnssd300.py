...
from PredictionHead import PredictionConvolutions
...
class SSD300(nn.Module):
    ...
    def forward(self, image):
        ...
        :return: 8478 locations and class scores (i.e. w.r.t each prior box) for each image
        """
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        true_locs = torch.zeros((batch_size, n_priors, 4), dtype=torch.float).to(device)  # (N, 8478, 4)
        true_classes = torch.zeros((batch_size, n_priors), dtype=torch.long).to(device)  # (N, 8478)

        # For each image
        for i in range(batch_size):
            n_objects = boxes[i].size(0)

            overlap = find_jaccard_overlap(boxes[i],self.priors_xy)  # (n_objects, 8478)

            # For each prior, find the object that has the maximum overlap
            overlap_for_each_prior, object_for_each_prior = overlap.max(dim=0)  # (8478)
            ...
            true_locs[i] = cxcy_to_gcxgcy(xy_to_cxcy(boxes[i][object_for_each_prior]), self.priors_cxcy)  # (8478, 4)

        # Identify priors that are positive (object/non-background)
        positive_priors = true_classes != 0  # (N, 8478)
        ...
        conf_loss_all = self.cross_entropy(predicted_scores.view(-1, n_classes), true_classes.view(-1))  # (N * 8478)
        conf_loss_all = conf_loss_all.view(batch_size, n_priors)  # (N, 8478)
        ...
        conf_loss_neg, _ = conf_loss_neg.sort(dim=1, descending=True)  # (N, 8478), sorted by decreasing hardness
        hardness_ranks = torch.LongTensor(range(n_priors)).unsqueeze(0).expand_as(conf_loss_neg).to(device)  # (N, 8478)
        hard_negatives = hardness_ranks < n_hard_negatives.unsqueeze(1)  # (N, 8478)
